...
def load_interpolation_input_data(
    input_file,
    elevation_var,
    x_var="lon",
    y_var="lat",
    ...
    if interpolation_flag_var and interpolation_flag_var not in ds.variables:
        raise ValueError(
            f"You set the interpolation flag, but variable {interpolation_flag_var} was not found in file {input_file}"
        )
    ...
    if interpolation_flag_var:
        ...
        if np.ma.is_masked(elevation):
            ...
            raise RuntimeError(
                "The elevation var on the input NetCDF does neither have an interpolation_flag variable nor a set of masked values. Therefore, there is nothing to interpolate!"
            )
    ds.close()
...
def write_interpolation_results(
    input_file,
    output_file,
    elevation,
    mask_int,
    ...
    if areas_kml_file or not interpolation_flag_var:
        ...
        if "interpolation_flag" not in out_ds.variables.keys():
            ...
            complib = (
                "zlib"
                if elev_var.filters() is not None
                and elev_var.filters().get("zlib", False)
                else None
            )
            complevel = (
                elev_var.filters().get("complevel", 0)
                if elev_var.filters() is not None
                else 0
            )
            chunksizes = (
                tuple(elev_var.chunking())
                if elev_var.chunking() is not None
                and elev_var.chunking() != "contiguous"
                else None
            )
...
def write_interpolation_results_new_file(
    output_file,
    elevation,
    mask_int,
    lats,
    ...
    lon[:] = lons
    elevation_var = out_ds.createVariable(
        elevation_var_name, np.float64, ("lat", "lon")
    )
    elevation_var[:, :] = elevation
    interpolation_flag_var = out_ds.createVariable(
        interpolation_flag_var_name, "int8", ("lat", "lon")
    )
...
def create_work_areas(elevation, areas_kml_file, lons_1d, lats_1d):
    ...
    if areas_kml_file:
        ...
        work_areas = np.full(
            (elevation.shape[0], elevation.shape[1], len(polys)), False
        )
        ...
        for poly in polys:
            ...
            color = 255
            area_mask = np.zeros(
                (elevation.shape[0], elevation.shape[1]), dtype="uint8"
            )
            ...
            if not np.any(work_areas[:, :, i]):
                raise ValueError(
                    "One of the polygon areas of the KML does not contain any point within the map!"
                )
