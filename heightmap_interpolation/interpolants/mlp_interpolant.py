...
class MLPScatteredInterpolator(nn.Module):
    ...
    def __init__(self, input_dim=2, hidden_dims=[64, 128, 128, 64], output_dim=1, 
                 use_fourier=False, num_frequencies=10, fourier_scale=1.0):
        super().__init__()
...
class MLPScatteredDataInterpolatorTrainer:
    ...
    def __init__(self, model, lr=1e-3, smoothness_weight=0.0, device=None):
        ...
        self.criterion = nn.MSELoss(reduction='none') # reduction='none' is necessary for per-sample weighting
    ...
    def compute_laplacian_loss(self, X):
        ...
        for i in range(X.shape[1]):
            grad_i = grads[:, i]
            second_deriv = torch.autograd.grad(grad_i.sum(), X, create_graph=True)[0][:, i]
    ...
    def fit(self, X_train, y_train, epochs=5000, 
            use_density_weighting=False, batch_sampling=None,
            verbose=True, show_loss_plots=False, verbose_every=500, loss_plots_every=500):
        """
        ...
        if use_density_weighting:
            ...
            sample_weights = self.compute_sample_weights(X_train, method='density', k=10)
        ...
        for epoch in range(epochs):
            ...
            if batch_sampling is not None and n_samples > 256:
                ...
                weights_batch = sample_weights[indices] if sample_weights is not None else None
            else:
                X_batch = X_train
                y_batch = y_train
            ...
            self.history['smoothness_loss'].append(smoothness_loss if isinstance(smoothness_loss, float) else smoothness_loss.item())
...
class MLPInterpolant(Interpolant):
    def __init__(self, x, y, z, # Interpolant basis
                 hidden_dims=[64, 128, 128, 128, 128, 64], output_dim=1, use_fourier=False, num_frequencies=10, fourier_scale=1.0, # Model params
                 lr=1e-3, smoothness_weight=0.0, use_density_weighting=True, epochs=10000, device=None): # Training params
