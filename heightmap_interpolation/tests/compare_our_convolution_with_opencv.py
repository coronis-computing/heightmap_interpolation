...
def my_fun(param):
    ...
    if len(image.shape) == 3:
        print(
            "[WARNING] This script expects single-channel images, we will convert the input color image to grayscale"
        )
    ...
    if param.mask:
        ...
        mask[:, 0 : (mask.shape[1] // 4) * 3] = (
            False  # half of the image should not be taken into account
        )
    ...
    filt = np.array([[0.0, 1.0, 0.0], [1.0, -4.0, 1.0], [0.0, 1.0, 0.0]])

    # Force numba compilation before timing
    res = nb_convolve_at_mask(
        np.random.random((3, 3)), np.ones((3, 3), dtype=bool), np.random.random((3, 3))
    )
...
def parse_args(args=None):
    ...
    parser.add_argument(
        "input_image", action="store", type=str, help="Input image file"
    )
    parser.add_argument(
        "--mask",
        "-m",
