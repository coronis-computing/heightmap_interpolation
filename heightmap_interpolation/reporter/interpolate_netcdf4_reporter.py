...
def create_default_config():
    ...
    for method in methods:
        ...
        test = {"name": "", "method": method, "parameters": defaults}
        tests.append(test)
...
class InterpolateNetCDF4Reporter:
    ...
    def write_intro_section(self):
        ...
        if not self.config["intro_text"]:
            f.write(
                "This report presents the execution results using interpolation methods of the `heightmap_interpolation` toolbox.\n\n"
            )
        else:
    ...
    def load_dataset(self, ds_config):
        # Read the file
        lats_mat, lons_mat, elevation, mask_int, mask_ref, work_areas = (
    ...
    def write_data_analysis_section(self, ds_config):
        ...
        if not self.ds_areas:
            if not np.all(
                np.logical_or(self.ds_reference_mask, self.ds_interpolate_mask)
            ):
                rgb_interpolate_mask = cv2.cvtColor(
                    np.asarray(self.ds_interpolate_mask, dtype="uint8") * 255,
                    cv2.COLOR_GRAY2RGB,
                )
                ref_or_interpolate_mask = np.logical_or(
                    self.ds_reference_mask, self.ds_interpolate_mask
                )
            ...
            for i in range(self.ds_work_areas.shape[2]):
                ...
                work_area_mask_cv = (
                    np.asarray(self.ds_work_areas[:, :, i], dtype="uint8") * 255
                )
                contours, hierarchy = cv2.findContours(
                    work_area_mask_cv, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
                )
        ...
        f.write(
            "| **Num. cells** | **Unknown cells** | **Known cells** | **\\% to interpolate** |\n"
            + "| :---: | :---: | :---: | :---: |\n"
            + "| {:d} | {:d} | {:d} | {:.2f} |\n\n".format(
                total_pixels,
                total_pixels - num_pixels_to_interpolate,
                num_pixels_to_interpolate,
                interpolate_percent,
            )
        )
    ...
    def run_test(self, tst_config):
        ...
        self.interpolation_results_netcdf = os.path.join(
            self.current_results_dir, self.test_base_name + ".nc"
        )
        ...
        cmd_file_path = os.path.join(
            self.current_results_dir, self.test_base_name + ".cmd"
        )
        ...
        log_file_path = os.path.join(
            self.current_results_dir, self.test_base_name + ".log"
        )
        ...
        test_stats = os.path.join(
            self.current_results_dir, self.test_base_name + "_stats.json"
        )
    ...
    def write_test_section(self, tst_config):
        ...
        if tst_config["parameters"]:
            f.write(
                "Executed the *{:s}* interpolation method with the following parameters (defaults not included):\n\n".format(
                    tst_config["method"]
                )
            )
            ...
            f.write(
                "Executed the *{:s}* interpolation method with the default parameters.\n\n".format(
                    tst_config["method"]
                )
            )
        ...
        f.write(
            "The results of the interpolation can be seen in figure \\ref{{{:s}}}.\n\n".format(
                tex_figure_label
            )
        )
        f.write(
        ...
        if overshooted_min or overshooted_max:
            f.write(
                "**WARNING**: the interpolated areas present some **overshooting** with respect to the original known input values:\n\n"
            )
        if overshooted_min:
            f.write(
                "* Original minimum elevation was {:.2f}, but in the interpolated result is {:.2f}\n".format(
                    self.ds_min_elevation, interpolated_elevation_min
                )
            )
        if overshooted_max:
            f.write(
                "* Original maximum elevation was {:.2f}, but in the interpolated result is {:.2f}\n".format(
                    self.ds_max_elevation, interpolated_elevation_max
                )
            )
        ...
        if stats["run_duration"] > 60:
            ...
            if duration > 60:
                ...
                duration_unit = "hours"
        f.write(
            "The execution took a total of **{:.2f} {}**. This timing includes the loading of the data, the interpolation itself and the writing of results to disk. Also, it required the following resources:\n\n".format(
    ...
    def write_tests_comparison_section(self, ds_config):
        self.section_dir = os.path.join(
            self.docs_dir, "dataset_{:d}".format(self.ds_counter), "tests_comparison"
        )
        ...
        f = open(self.doc_path, "w")
        f.write(
            "## Comparison between tests performed to Dataset {:d}".format(
                self.ds_counter
            )
        )
        if ds_config["name"]:
        ...
        f.write(
            "The following table presents at a glance some of the statistics computed for the different methods/tests applied to the present dataset:\n\n"
        )

        # Table header
        f.write(
            "| **Test Num.** | **Method** | **Run time (s) ** | **Mean CPU %** | **Max. CPU %** | **Total CPU time (s)** | **Mean Memory (MB)** | **Max Memory (MB)** |\n"
            + "| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |\n"
        )
        self.test_counter = 0
    ...
    def render_report_to_pdf(self):
        ...
        pandoc_templates_dir = os.path.join(
            os.path.dirname(
                sys.modules[
                    "heightmap_interpolation.reporter.interpolate_netcdf4_reporter"
                ].__file__
            ),
            "templates",
        )
        ...
        shutil.copyfile(
            os.path.join(pandoc_templates_dir, "header.tex"), header_template_path
        )
        ...
        if ret_code != 0:
            raise ValueError(
                "Rendering the report to a PDF failed with return code = {:d}".format(
                    ret_code
                )
            )
        ...
        if self.config["output_tex_file"]:
            args = [
                ...
                "-s",
                "-o",
                self.config["output_tex_file"],
            ]
            ret_code = subprocess.call(args)

            if ret_code != 0:
                raise ValueError(
                    "Rendering the report to a TEX file failed with return code = {:d}".format(
                        ret_code
                    )
                )
